...
public class ServidorRestAdapter {
    ...
    public ResponseEntity<ServidorEfetivoResponse> createServidorEfetivo(@ModelAttribute @Valid ServidorEfetivoRequest request) {
        return ResponseEntity.status(HttpStatus.CREATED).body(efetivoService.create(request));
    }

    @PutMapping(value = "/efetivo/{id}", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public ResponseEntity<ServidorEfetivoResponse> updateServidorEfetivo(
            @PathVariable Long id,
            @ModelAttribute ServidorEfetivoRequest request) {
        return ResponseEntity.ok(efetivoService.update(id, request));
    }

    @DeleteMapping("/efetivo/{id}")
    public ResponseEntity<Void> delete(@PathVariable Long id) {
        efetivoService.delete(id);
